...
Every DOM `event` comes with a `preventDefault` property. `preventDefault` is a function that, when
...
Try clicking on a node that's not so deeply nested -- you should still see the event bubble up,
